...
### Finding Redpanda Documentation
...
2. **Sitemap**: `{site-url}/sitemap.xml` - Contains all documentation pages including this file and all exports. The sitemap index includes `{site-url}/sitemap-llms.xml` with all AI-optimized exports.

3. **MCP Server**: `{site-url}/mcp` - For Claude Code and MCP-compatible agents, this provides interactive documentation tools (see below).

### Why llms.txt Matters

llms.txt represents a minimum barrier for entry for documentation in an LLM consumption model. Many documentation sites don't provide structured AI-accessible formats, making it difficult for agents to discover and use documentation effectively. Redpanda provides multiple AI-optimized formats to ensure agents can always find what they need.

### Interactive Documentation Tools (MCP Server)

**{site-url}/mcp** - Model Context Protocol server for Claude Code

The Redpanda Documentation MCP server provides AI agents with interactive tools for:

- **Generating reference documentation**: Property docs, metrics, CLI commands, connectors
- **Accessing versioned content**: Redpanda versions, Console versions, API specifications
- **Querying documentation structure**: Navigate components, modules, and pages
- **Validating configurations**: Check compatibility and best practices
