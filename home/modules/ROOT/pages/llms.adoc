...
All documentation pages are available in Markdown format with proper filenames and YAML frontmatter. You can access Markdown content in two ways:

1. **Content negotiation**: Request any HTML URL with the `Accept: text/markdown` header to receive the Markdown source.
2. **Direct access**: Replace `.html` with `.md` in any URL (for example `{site-url}/current/home.html` → `{site-url}/current/home.md`).

### AI-Optimized Formats

#### Interactive MCP Server

**{site-url}/mcp[{site-url}/mcp]** - Model Context Protocol server for Claude Code
...
- **Access versioned content**: Redpanda versions, Console versions, API specifications
- **Query documentation structure**: Navigate components, modules, and pages
- **Validate configurations**: Check compatibility and best practices
...
For AI agents and tools, we provide multiple optimized static formats:

- **{site-url}/llms.txt[{site-url}/llms.txt]**: Curated overview following the llms.txt standard - start here for a quick introduction
- **{site-url}/llms-full.txt[{site-url}/llms-full.txt]**: Complete documentation export with all components
- **Component-specific exports**: Focused documentation for individual products:
  * {site-url}/ROOT-full.txt[{site-url}/ROOT-full.txt]: Redpanda Self-Managed documentation
  * {site-url}/redpanda-cloud-full.txt[{site-url}/redpanda-cloud-full.txt]: Redpanda Cloud documentation
  * {site-url}/redpanda-connect-full.txt[{site-url}/redpanda-connect-full.txt]: Redpanda Connect documentation
  * {site-url}/redpanda-labs-full.txt[{site-url}/redpanda-labs-full.txt]: Labs and tutorials
- **Individual Markdown pages**: Each HTML page has a corresponding `.md` file with YAML frontmatter containing page metadata (title, description, categories, etc.)
...
- {site-url}/current/get-started/quickstarts.md[Get started]: Fast-track your journey with Redpanda Self-Managed.
...
- **Previous versions**: Previous versions can be accessed by replacing `current` with the version number in any URL (for example `{site-url}/24.1/home.md`, `{site-url}/25.1/deploy.md`).
