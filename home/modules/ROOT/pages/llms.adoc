...
All documentation pages are available in markdown format with proper filenames and YAML frontmatter. You can access markdown content through content negotiation by requesting any HTML URL with the `Accept: text/markdown` header to receive the markdown source, or through direct access by replacing `.html` with `.md` in any URL (for example `{site-url}/current/home.html` becomes `{site-url}/current/home.md`).

### AI-Optimized Formats

For AI agents and tools, we provide multiple optimized formats. Start with **{site-url}/llms.txt[{site-url}/llms.txt]**, a curated overview following the llms.txt standard that provides a quick introduction to our documentation structure. For comprehensive access, **{site-url}/llms-full.txt[{site-url}/llms-full.txt]** contains a complete documentation export with all components.

We also offer component-specific exports for focused documentation access. **{site-url}/ROOT-full.txt[{site-url}/ROOT-full.txt]** contains Redpanda Self-Managed documentation, **{site-url}/redpanda-cloud-full.txt[{site-url}/redpanda-cloud-full.txt]** covers Redpanda Cloud, **{site-url}/redpanda-connect-full.txt[{site-url}/redpanda-connect-full.txt]** provides Redpanda Connect documentation, and **{site-url}/redpanda-labs-full.txt[{site-url}/redpanda-labs-full.txt]** includes labs and tutorials.

Each HTML page has a corresponding `.md` file with YAML frontmatter containing page metadata including title, description, and categories.
