...
+
`include::streaming:get-started:partner-integration.adoc[tag=single-source]`
+
...
`include::streaming:reference:properties/cluster-properties.adoc[tags=audit_excluded_principals]`
...
This displays a blue "beta" badge on the page with the tooltip text: "{page-beta-text}"
...
This displays a "Limited Availability" pill-shaped badge on the page with the tooltip text: "{page-limited-availability-text}"
...
* **Left navigation**: Next to the page title in the navigation tree
  - Beta: Displays as `(beta)` text
  - Limited Availability: Displays as an orange pill badge

NOTE: You should only use one status badge per page. Do not combine `page-beta` and `page-limited-availability` on the same page.
