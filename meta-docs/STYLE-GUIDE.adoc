...
The tabs functionality is provided by the `asciidoctor-tabs` module. For more details about using tabs, see the https://github.com/asciidoctor/asciidoctor-tabs[module's GitHub repository].
...
During the build, the macro collects all the terms used in `glossterm` macros, sorts the alphabetically, and adds them to the page that includes the `glossary` macro. Any terms without a `link` property link to the glossary page.

== Link macros

The Redpanda documentation includes custom macros to make it easier for writers to link to common pages.

Using a macro to generate URLs provides the following benefits:
