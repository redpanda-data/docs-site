...
async function scrapeAndIndex() {
  ...
  for (const path of DOC_PATHS) {
    ...
    try {
      ...
      console.log(`📄 Found ${pageInfo.groups.length} groups and ${pageInfo.frameUrls.length} operations`);
      ...
      for (const frameInfo of pageInfo.frameUrls) {
        ...
        try {
           ...
           * 
           * Fallback behavior:
           * - If title is missing, we generate one from operationId (operation-get_brokers → "Get Brokers")
          ...
          console.error(`⚠️  Error processing ${frameInfo.operationId}: ${err.message}`);
          errorCount++;
      ...
      console.log(`✅ Processed ${successCount} operations successfully, ${errorCount} with errors`);
  ...
  try {
    ...
    console.log(`\n🎉 Successfully indexed ${objectIDs.length} records to Algolia!`);

    // Verify indexing success rate
    const successRate = ((summary.withMethod / summary.endpoints) * 100).toFixed(1);
    console.log(`📈 Method extraction success rate: ${successRate}%`);
    
    if (summary.withMethod === summary.endpoints) {
      console.log('✅ Perfect! All endpoints have method information.');
    } else {
      console.log(`⚠️  ${summary.endpoints - summary.withMethod} endpoints missing method information.`);
    }
